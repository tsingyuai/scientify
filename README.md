...
**研究问题**：在论文采用的黑洞模型中，改变参数会让系统经历哪些变化？这些变化能否用一个统一的规律解释和预测？

- **从数据中发现规律**：Scientify 分析不同参数下的数值计算结果，识别出系统变化的四个阶段，并定位了阶段之间的三个分界点，揭示出完整的演变顺序。
- **建立可预测的模型**：Scientify 将两个参数合并为一个关键变量，推导出三个分界点随压力变化的统一规律：压力增大到原来的 4 倍，对应的分界参数值降为一半。它还给出了最后一个分界点的精确公式，让模型能够直接计算变化何时发生。
- **用计算验证模型**：Scientify 将三个不同压力下的计算结果与理论预测对照，验证它们遵循同一缩放规律，并结合稳定性分析判断哪些状态可以实际稳定存在。由此发现，曲线首次改变形态后，两种黑洞状态仍可稳定共存。
